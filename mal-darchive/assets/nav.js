...
(function () {
  ...
  var stamps = Array.prototype.slice.call(document.querySelectorAll(".stamp-in"));
  if (stamps.length) {
    if (reduced || !("IntersectionObserver" in window)) {
      stamps.forEach(function (s) { s.classList.add("stamped"); });
    } else {
  ...
  /* ---- 塗黑檔案：點按切換顯影（觸控裝置用；滑鼠 hover 由 CSS 處理） ---- */
  Array.prototype.forEach.call(document.querySelectorAll(".redacted"), function (r) {
    if (!r.hasAttribute("tabindex")) r.setAttribute("tabindex", "0");
    r.setAttribute("role", "button");
    r.setAttribute("aria-label", "塗黑段落，點按顯影");
    r.addEventListener("click", function () { r.classList.toggle("revealed"); });
    r.addEventListener("keydown", function (e) {
      if (e.key === "Enter" || e.key === " ") {
        e.preventDefault(); r.classList.toggle("revealed");
      }
    });
  });
